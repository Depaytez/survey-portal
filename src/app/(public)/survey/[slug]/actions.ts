...
import { isHoneypotValueTripped, isRenderedAtTooRecent } from "@/lib/spam-protection";
...
export type SpamCheck = { website: string; renderedAt: number };
...
export async function submitSurveyResponse(
  ...
  // Same bot-resistance as the contact/stakeholder-interest forms. Report
  // success without writing anything — don't tip off automated submitters.
  if (isHoneypotValueTripped(spamCheck.website) || isRenderedAtTooRecent(spamCheck.renderedAt)) {
    return { success: true };
  }
  ...
  const answers_: { question_id: string; value: Json }[] = allQuestions
    .filter((question) => parsed.data[question.id] !== undefined && parsed.data[question.id] !== null)
    .map((question) => ({
      question_id: question.id,
      value: parsed.data[question.id] as Json,
    }));

  // A single RPC, not two separate inserts: submit_survey_response wraps
  // both writes in one transaction, so a failure partway through can't
  // leave a response row with no answers behind (that used to silently
  // inflate every response count).
  ...
  if (error) {
    console.error("Failed to submit survey response:", error.message);
