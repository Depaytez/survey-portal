...
export async function updateStakeholderStatus(
  ...
  const { error } = await supabase.from("stakeholder_requests").update({ status }).eq("id", id);

  if (error) {
    console.error("Failed to update stakeholder request status:", error.message);
    return { success: false, error: "Something went wrong updating the status." };
  }

  revalidatePath("/admin/stakeholder-requests");
  return { success: true };
}
